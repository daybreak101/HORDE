...
public class Inventory {
	...
	public Inventory(Handler handler, Player player) {
		...
		arsenal = new Gun[6];
		arsenal[0] = new Glock17(handler, player);
		arsenal[1] = null;
		arsenal[2] = null;
	...
	public boolean throwGrenade(int fuseTime) {
		if (grenadeReadyTimer.isReady() && grenades > 0) {
			...
			handler.getWorld().getEntityManager().addEntity(
					new Grenade(handler, player.getCenterX(), player.getCenterY(), player.getZ(), false, destX, destY, 0, player, fuseTime));
			if (phd >= 1) {
				handler.getWorld().getEntityManager().addEntity(new Grenade(handler, player.getCenterX() + 20,
						player.getCenterY() + 20, player.getZ(), false, destX, destY, 0, player, fuseTime));
				handler.getWorld().getEntityManager().addEntity(new Grenade(handler, player.getCenterX() - 20,
						player.getCenterY() - 20, player.getZ(), false, destX, destY, 0, player, fuseTime));
			}
			if (player.getPeer() != null) {
				player.getPeer().sendNewGrenade(player.getUsername(), GunVars.GRENADE_NAME, destX, destY, player.getZ());
			}
			return true;
		}
		return false;
		...
		if (grenadeReadyTimer.isReady() && specialGrenadeAmt > 0 && specialGrenadeType != -1) {
			...
			case 0:
				...
				handler.getWorld().getEntityManager()
						.addEntity(new GasGrenade(handler, player.getCenterX(), player.getCenterY(), player.getZ(),
				...
				if (player.getPeer() != null) {
					player.getPeer().sendNewGrenade(player.getUsername(), GunVars.GAS_GRENADE_NAME, destX, destY, player.getZ());
				}
				break;
			}
