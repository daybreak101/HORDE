...
public class Zombie extends Creature {
	...
	public Zombie(Handler handler, int id, float x, float y, int z, float speed, int health) {
	...
	public void tick() {
		...
		if (freezeStatus.isFrozen()) {
			...
			closestPlayer = handler.getClosestPlayerToZombie(getCenterX(), getCenterY(), getZ());
			if(closestPlayer == null) {
				closestPlayer = handler.getCurrentPlayer();
			}
			delaySource.tick();
			if (reachedGoal || delaySource.isReady())
				source = handler.getWorld().getPathingLogic().getClosestNode(getCenterX(), getCenterY(), getZ());
...
//		}
		...
		if (z == current.getZ() && !justAttacked && current.getHealth() > 0 && getHitBox(0, 0).intersects(current.getHitbox())
		...
		if (isAttacking) {
			...
			for (Barrier e : handler.getWorld().getEntityManager().getBarriers()) {
				if (z == e.getZ() && e.getCollisionBounds(0, 0).intersects(this.getHitBox(0, 0))) {
	...
	public boolean isCritical(Player player) {
		if (player.getInv().getBlessings().getBlessing() == "So No Head?" && player.getInv().getBlessings().isRunning())
			return true;
	...
	public void takeOnlineDamage(int amount) {
		...
		if (freezeStatus.isFrozen()) {
			...
			active = false;
			handler.getWorld().getEntityManager().addBlood(new Blood(handler, x, y, z, zombieType));
	...
	public boolean checkForObstacles() {
		...
		for (int i = 1; i < 5; i++) {
			for (InteractableStaticEntity e : handler.getWorld().getEntityManager().getInteractables()) {
				if (z == e.getZ() && z2p[i].intersects(e.getCollisionBounds(0, 0)))
					if (!handler.getWorld().getEntityManager().getBarriers().contains(e))
						return true;
			}
			for (Wall e : handler.getWorld().getEntityManager().getWalls()) {
	...
	public void powerupChecker() {
		int vertex = handler.getWorld().getPathingLogic().getClosestNode((int) (getCenterX()), (int) (getCenterY()), getZ());
		// Node node = ;
		if (handler.getWorld().getPathingLogic().getNodes().get(vertex).withinPlayable()) {
			if (handler.getRoundLogic().getPowerups().isPowerUpReady()) {
				handler.getWorld().getEntityManager()
						.addPowerUp(handler.getRoundLogic().getPowerups().generatePowerUp(x, y, z));
			}
...
//			}
	...
	public void dieByGoodBoy() {
		...
		if (handler.getCurrentPlayer().getInv().getLuna() >= 1) {
			smallSpawned = handler.getRoundLogic().getPowerups().spawnMiniByLuna(Math.round(x), Math.round(y), z);
		}

		if (handler.getCurrentPlayer().getInv().getLuna() == 3 && !smallSpawned) {
			handler.getRoundLogic().getPowerups().spawnByLuna(Math.round(x), Math.round(y), z);
		}
		active = false;
	}
	...
	public boolean checkEntityCollisions(float xOffset, float yOffset) {
		if (z == handler.getCurrentPlayer().getZ() && handler.getCurrentPlayer().getCollisionBounds(0, 0).intersects(getCollisionBounds(xOffset, yOffset))) {
			return true;
		}
