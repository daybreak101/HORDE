...
public class PowerUpManager {
	...
	public boolean spawnMiniByLuna(int x, int y, int z) {
		...
		if (rng < 40) {
			...
			case 0:
				...
				.addPowerUp(new LunaClip(handler, totalPowerups, x, y, z));
				break;
			case 1:
				handler.getWorld().getEntityManager()
				.addPowerUp(new LunaHealth(handler, totalPowerups, x, y, z));
				break;
			case 2: 
				handler.getWorld().getEntityManager()
				.addPowerUp(new LunaPoints(handler, totalPowerups, x, y, z));
				break;
			}
			return true;
		}
	...
	public void spawnByLuna(int x, int y, int z) {
...
//		int rng = RandomUtil.nextInt(0, 200);
	...
	public PowerUps generatePowerUp(float fx, float fy, int z) {
		if (handler.getCurrentPlayer().getPeer() == null || handler.getCurrentPlayer().getPeer().isServer()) {
			int x = Math.round(fx);
			int y = Math.round(fy);
			Random rand = new Random();
			...
			case 0:
				if (maxAmmo) {
					return generatePowerUp(x, y, z);
				} else {
					maxAmmo = true;
					powerupsSpawned++;
					totalPowerups++;
					if (handler.getCurrentPlayer().getPeer() != null)
						handler.getCurrentPlayer().getPeer().sendNewPowerUp("maxAmmo", totalPowerups, x, y, z);
					return new MaxAmmo(handler, totalPowerups, x, y, z);
				}
			case 1:
				if (nuke) {
					return generatePowerUp(x, y, z);
					...
					totalPowerups++;
					if (handler.getCurrentPlayer().getPeer() != null)
						handler.getCurrentPlayer().getPeer().sendNewPowerUp("nuke", totalPowerups, x, y, z);
					return new Nuke(handler, totalPowerups, x, y, z);
				}
			case 2:
				if (doublePoints) {
					return generatePowerUp(x, y, z);
					...
					totalPowerups++;
					if (handler.getCurrentPlayer().getPeer() != null)
						handler.getCurrentPlayer().getPeer().sendNewPowerUp("doublePoints", totalPowerups, x, y, z);
					return new DoublePoints(handler, totalPowerups, x, y, z);
				}
			case 3:
				if (instakill) {
					return generatePowerUp(x, y, z);
					...
					totalPowerups++;
					if (handler.getCurrentPlayer().getPeer() != null)
						handler.getCurrentPlayer().getPeer().sendNewPowerUp("instakill", totalPowerups, x, y, z);
					return new InstaKill(handler, totalPowerups, x, y, z);
				}
			case 4:
				if (infiniteAmmo) {
					return generatePowerUp(x, y, z);
					...
					totalPowerups++;
					if (handler.getCurrentPlayer().getPeer() != null)
						handler.getCurrentPlayer().getPeer().sendNewPowerUp("infiniteAmmo", totalPowerups, x, y, z);
					return new InfiniteAmmo(handler, totalPowerups, x, y, z);
				}
			case 5:
				if (deathMachine) {
					return generatePowerUp(x, y, z);
					...
					totalPowerups++;
					if (handler.getCurrentPlayer().getPeer() != null)
						handler.getCurrentPlayer().getPeer().sendNewPowerUp("deathMachine", totalPowerups, x, y, z);
					return new DeathMachine(handler, totalPowerups, x, y, z);
				}
			case 6:
				totalPowerups++;
				if (handler.getCurrentPlayer().getPeer() != null)
					handler.getCurrentPlayer().getPeer().sendNewPowerUp("healthUp", totalPowerups, x, y, z);
				return new HealthUp(handler, totalPowerups, x, y, z);
			}
		}
		return null;
	...
	public void forceSpawnDoublePoints(int x, int y, int z) {
		totalPowerups++;
		if (handler.getCurrentPlayer().getPeer() != null) {
			if (!handler.getCurrentPlayer().getPeer().isServer()) {
				handler.getCurrentPlayer().getPeer().sendUserSpawnedPowerup("doublePoints", x, y, z);
			} else {
				em.addPowerUp(new DoublePoints(handler, totalPowerups, x, y, z));
				handler.getCurrentPlayer().getPeer().sendNewPowerUp("doublePoints", totalPowerups, x, y, z);
			}
		} else {
			em.addPowerUp(new DoublePoints(handler, totalPowerups, x, y, z));
	...
	public void forceSpawnNuke(int x, int y, int z) {
		totalPowerups++;
		if (handler.getCurrentPlayer().getPeer() != null) {
			if (!handler.getCurrentPlayer().getPeer().isServer()) {
				handler.getCurrentPlayer().getPeer().sendUserSpawnedPowerup("nuke", x, y, z);
			} else {
				em.addPowerUp(new Nuke(handler, totalPowerups, x, y, z));
				handler.getCurrentPlayer().getPeer().sendNewPowerUp("nuke", totalPowerups, x, y, z);
			}
		} else {
			em.addPowerUp(new Nuke(handler, totalPowerups, handler.getCurrentPlayer().getX(),
					handler.getCurrentPlayer().getY(), handler.getCurrentPlayer().getZ()));
		}
	}

	public void forceSpawnMaxAmmo(int x, int y, int z) {
		totalPowerups++;
		if (handler.getCurrentPlayer().getPeer() != null) {
			if (!handler.getCurrentPlayer().getPeer().isServer()) {
				handler.getCurrentPlayer().getPeer().sendUserSpawnedPowerup("maxAmmo", x, y, z);
			} else {
				em.addPowerUp(new MaxAmmo(handler, totalPowerups, x, y, z));
				handler.getCurrentPlayer().getPeer().sendNewPowerUp("maxAmmo", totalPowerups, x, y, z);
			}
		} else {
			em.addPowerUp(new MaxAmmo(handler, totalPowerups, x, y, z));
	...
	public void forceSpawnInfiniteAmmo(int x, int y, int z) {
		totalPowerups++;
		if (handler.getCurrentPlayer().getPeer() != null) {
			if (!handler.getCurrentPlayer().getPeer().isServer()) {
				handler.getCurrentPlayer().getPeer().sendUserSpawnedPowerup("infiniteAmmo", x, y, z);

			} else {
				em.addPowerUp(new InfiniteAmmo(handler, totalPowerups, x, y, z));
				handler.getCurrentPlayer().getPeer().sendNewPowerUp("infiniteAmmo", totalPowerups, x, y, z);
			}
		} else {
			em.addPowerUp(new InfiniteAmmo(handler, totalPowerups, x, y, z));
	...
	public void forceSpawnInstaKill(int x, int y, int z) {
		totalPowerups++;
		if (handler.getCurrentPlayer().getPeer() != null) {
			if (!handler.getCurrentPlayer().getPeer().isServer()) {
				handler.getCurrentPlayer().getPeer().sendUserSpawnedPowerup("instakill", x, y, z);

			} else {
				em.addPowerUp(new InstaKill(handler, totalPowerups, x, y, z));
				handler.getCurrentPlayer().getPeer().sendNewPowerUp("instakill", totalPowerups, x, y, z);
			}
		} else {
			em.addPowerUp(new InstaKill(handler, totalPowerups, x, y, z));
	...
	public void forceSpawnHealthUp(int x, int y, int z) {
		totalPowerups++;
		if (handler.getCurrentPlayer().getPeer() != null) {
			if (!handler.getCurrentPlayer().getPeer().isServer()) {
				handler.getCurrentPlayer().getPeer().sendUserSpawnedPowerup("healthUp", x, y, z);

			} else {
				em.addPowerUp(new HealthUp(handler, totalPowerups, x, y, z));
				handler.getCurrentPlayer().getPeer().sendNewPowerUp("healthUp", totalPowerups, x, y, z);
			}
		} else {
			em.addPowerUp(new HealthUp(handler, totalPowerups, x, y, z));
	...
	public void forceSpawnDeathMachine(int x, int y, int z) {
		totalPowerups++;
		if (handler.getCurrentPlayer().getPeer() != null) {
			if (!handler.getCurrentPlayer().getPeer().isServer()) {
				handler.getCurrentPlayer().getPeer().sendUserSpawnedPowerup("deathMachine", x, y, z);

			} else {
				em.addPowerUp(new DeathMachine(handler, totalPowerups, x, y, z));
				handler.getCurrentPlayer().getPeer().sendNewPowerUp("deathMachine", totalPowerups, x, y, z);
			}
		} else {
			em.addPowerUp(new DeathMachine(handler, totalPowerups, x, y, z));
	...
	public void forceSpawnPerkBag(int x, int y, int z) {
		totalPowerups++;
		if (handler.getCurrentPlayer().getPeer() != null) {
			if (!handler.getCurrentPlayer().getPeer().isServer()) {
				handler.getCurrentPlayer().getPeer().sendUserSpawnedPowerup("perkBag", x, y, z);

			} else {
				em.addPowerUp(new PerkBag(handler, totalPowerups, x, y, z));
				handler.getCurrentPlayer().getPeer().sendNewPowerUp("perkBag", totalPowerups, x, y, z);
			}
		} else {
			em.addPowerUp(new PerkBag(handler, totalPowerups, x, y, z));
		}
