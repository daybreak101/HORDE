...
public class NewFlameBullet extends Bullet {
	...
	public NewFlameBullet(Handler handler, float x, float y, int z, int range, Gun gun, int frame) {
...
//		}
		for (Zombie e : handler.getWorld().getEntityManager().getZombies()) {
			if (z == e.getZ() && !zombiesHit.contains(e) && cb.intersects(e.getHitBox(0, 0))) {
	...
	public boolean checkForWallImpact() {
		...
		for (Wall e : handler.getWorld().getEntityManager().getWalls()) {
			if (z == e.getZ() && cb.intersects(e.getCollisionBounds(0, 0))) {
				return true;
			}
		}
		return false;
