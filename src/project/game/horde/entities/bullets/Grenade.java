...
public class Grenade extends Bullet {
	...
	//new normal grenades
	public Grenade(Handler handler, float x, float y, int z, boolean isImpact, float destX, float destY, double radianOffset, Player player, int fuseStart) {
		super(handler, x, y, z, 10000, player);
		x = (int) x;
		y = (int) y;
		width = 10;
		height = 10;
	...
	// normal grenades
	public Grenade(Handler handler, float x, float y, int z, boolean isImpact, float destX, float destY, double radianOffset, Player player) {
		super(handler, x, y, z, 10000, player);
		x = (int) x;
		y = (int) y;
		width = 10;
	...
	public Grenade(Handler handler, float x, float y, int z, boolean isImpact, float destX, float destY, Player player, Gun gun) {
		super(handler, x, y, z, 10000, player);
		x = (int) x;
		y = (int) y;
		width = 10;
...
//	// idk
	public Grenade(Handler handler, float x, float y, int z, Color color) {
		super(handler, x, y, z, 10000, color);
		width = 10;
		height = 10;
		counter = 0;
		timer = 7;
...
//			}
	...
	public void findEntitiesInRadius() {
		...
		for (Zombie f : handler.getWorld().getEntityManager().getZombies()) {
			...
			if (z == f.getZ() && explosionRadius.intersects(f.getHitBox(0, 0))) {
		...
		if (z == player.getZ() && explosionRadius.intersects(player.getCollisionBounds(0, 0))) {
