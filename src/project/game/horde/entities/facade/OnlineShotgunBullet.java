...
public class OnlineShotgunBullet extends OnlineBullet{
	...
	public OnlineShotgunBullet(Handler handler, float x, float y, int z, int range, float angle, double spread, int pelletNum, boolean isUpgraded) {
		super(handler, x, y, z, range, (float) angle, isUpgraded);
	
		pellets = new ArrayList<OnlineBullet>(pelletNum);
		for(int i = 0; i < pelletNum; i++) {
			if(i == 0)
				pellets.add(new OnlineBullet(handler, x, y, z, range, (float) angle, isUpgraded));
			else{
				...
				pellets.add(new OnlineBullet(handler, x, y, z, range, (float) angle , actualSpread, isUpgraded));
			}
		}
