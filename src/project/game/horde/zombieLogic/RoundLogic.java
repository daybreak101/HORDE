...
public class RoundLogic {
    ...
    public RoundLogic(Handler handler, ArrayList<Spawner> spawners, String map) {
    ...
    public void tick() {
        if (test) {
            ...
            if (zombiesLeft == 0 && handler.getWorld().getEntityManager().getZombies().size() == 0) {
                ...
                counter++;
				//skip round delay in uni map
                if ("uni".equals(map)) {
                    handler.getProgression().gainXP(currentRound * 100);
                    nextRound();
                } else if (counter >= roundCooldown) {
                    counter = 0;
                    handler.getProgression().gainXP(currentRound * 100);
                    nextRound();
                }
...
//				}
        ...
        // zpr = 1;
        // }
        else if (!"test".equals(map) && currentRound == nextDogRound) {
            zpr = 6;
            isDogRound = true;
        } else {
            isDogRound = false;
