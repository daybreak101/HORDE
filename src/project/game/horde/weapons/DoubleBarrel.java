...
public class DoubleBarrel extends Gun {
    ...
    public DoubleBarrel(Handler handler, Player player) {
        super(handler, player, GunVars.DB_DAMAGE, GunVars.DB_FIRERATE, GunVars.DB_RELOADSPEED, GunVars.DB_GUNCLIP,
                GunVars.DB_MAXRESERVE, GunVars.DB_WEIGHT, GunVars.DB_RANGE, 30);
    ...
    public void shootOnline(int x, int y, float angle, float volume) {
        handler.getWorld().getEntityManager()
                .addEntity(new OnlineShotgunBullet(handler, x, y, range,
                        angle, GunVars.DB_PELLET_SPREAD, GunVars.DB_PELLET_COUNT, isUpgraded));

        if (isUpgraded) {
            Sounds.playClip(GunSounds.UPGRADED_ID, 1, volume, false);
        }
        Sounds.playClip(GunSounds.DB_SHOT_ID, 1, volume, false);
    }
