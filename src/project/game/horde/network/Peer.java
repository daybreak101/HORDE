...
//				}
	...
	public void sendNewPowerUp(String powerup, int id, int x, int y, int z) {
		if (isServer) {
			server.sendToAllTCP(new Message(Message.SPAWN_POWERUP, powerup, id, x, y, z));
		}
	}

	public void sendUserSpawnedPowerup(String powerup, int x, int y, int z) {
		if (!isServer) {
			client.sendTCP(new Message(Message.USER_SPAWNED_POWERUP, powerup, 0, x, y, z));
		}
	}
	...
	public void sendNewGrenade(String username, String grenade, int destX, int destY, int z) {
		if (isServer) {
			server.sendToAllTCP(new Message(Message.USER_GRENADE_TOSS, username, grenade, destX, destY, z));
		} else {
			client.sendTCP(new Message(Message.USER_GRENADE_TOSS, username, grenade, destX, destY, z));
		}
	}

	public void sendPlayerGrenadeLauncherShot(String username, int destX, int destY, int z) {
		if (isServer) {
			server.sendToAllTCP(new Message(Message.USER_SHOT_GRENADE_LAUNCHER, username, "", destX, destY, z));
		} else {
			client.sendTCP(new Message(Message.USER_SHOT_GRENADE_LAUNCHER, username, "", destX, destY, z));
		}
	}
	...
	public void sendNewLunaCoords(String username, float x, float y, int z, float angle) {
		if (isServer) {
			server.sendToAllTCP(new Message(Message.USER_LUNA_MOVED, username, Math.round(x), Math.round(y), z, angle));
		} else {
			client.sendTCP(new Message(Message.USER_LUNA_MOVED, username, Math.round(x), Math.round(y), z, angle));
		}
	}
